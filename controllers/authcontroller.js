...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  //4. Check if user  changed pssword after token was issued.
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(new AppError('You dont have the access to do this', 403));
    }
    next();
  };
};
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  //2. Set the new password if token has not expired and there is a user, set a new password
  if (!user) {
    return next(new AppError('Token has expired or incorrect', 400));
  }
  ...
  user.PasswordResetExpires = undefined;
  await user.save();
  //3. Update the update the changed password at property for the user
